...
class EnhancedLogger:
    ...
    def log_ml_status(message: str, confidence: float = None, emoji: str = "🤖"):
        ...
        if confidence is not None:
            ...
            logger.info(
                f"{emoji} {conf_emoji} {message} (Confidence: {confidence:.2%})"
            )
        else:
            logger.info(f"{emoji} {message}")
...
class MLOptimizer:
    ...
    def load_models(self):
        ...
        try:
            ...
            if os.path.exists(self.study_path):
                ...
                models_loaded += 1
                EnhancedLogger.log_success(
                    f"Optuna Study loaded ({len(self.study.trials)} trials)", "🔬"
                )
            else:
            ...
            if models_loaded > 0:
                EnhancedLogger.log_success(
                    f"Loaded {models_loaded}/3 ML components", "✨"
                )
    ...
    def create_features(self, dataframe: DataFrame) -> np.ndarray:
        ...
        try:
            ...
            if "fisher" not in dataframe.columns or dataframe["fisher"].isna().all():
                EnhancedLogger.log_warning(
                    "Fisher Transform missing, calculating...", "🎯"
                )
                ...
                for i in range(period, len(dataframe)):
                    ...
                    if price_max != price_min:
                        norm = (median_price.iloc[i] - price_min) / (
                            price_max - price_min
                        )
                        norm = 2 * norm - 1
            ...
            if "baseline_diff" not in dataframe.columns:
                EnhancedLogger.log_warning(
                    "Baseline diff missing, calculating...", "📈"
                )
                try:
                    from pandas_ta import ema
            ...
            if not pd.isna(baseline_diff_mean) and not pd.isna(baseline_diff_sum):
                combined_baseline = (
                    baseline_diff_mean + baseline_diff_sum * 0.1
            ...
            if len(features) != 12:
                EnhancedLogger.log_error(
                    f"Feature count error: Expected 12, got {len(features)}", "❌"
                )
            ...
            EnhancedLogger.log_success(
                f"Created exactly {len(features)} ML features", "✨"
            )
            ...
            if len(features) == 12:
                ...
                EnhancedLogger.log_error(
                    f"Feature count still wrong: {len(features)}/12", "❌"
                )
    ...
    def optimize_parameters(self, dataframe: DataFrame, current_performance: float):
        ...
        def objective(trial):
            ...
            fisher_buy_threshold = trial.suggest_float(
                "fisher_buy_threshold", -1.0, 2.5
            )
            ...
            fisher_sell_threshold = trial.suggest_float(
                "fisher_sell_threshold", 2.0, 3.9
            )
            ...
            if len(self.study.trials) % 5 == 0:
                ...
                EnhancedLogger.log_parameter(
                    "Long Exit", f"{fisher_long_exit:.3f}", "📤"
                )
                EnhancedLogger.log_parameter(
                    "Short Exit", f"{fisher_short_exit:.3f}", "📤"
                )
        ...
        EnhancedLogger.log_parameter(
            "Optimization Time", f"{optimization_time:.1f}s", "⏱️"
        )
        ...
        if missing_params:
            EnhancedLogger.log_warning(
                f"Added defaults for missing params: {missing_params}", "🔧"
            )

        EnhancedLogger.log_success(
            f"Parameters verified: {len(best_params)} total", "✅"
        )
    ...
    def simulate_performance(self, dataframe: DataFrame, params: dict) -> float:
        """Simulate strategy performance with enhanced logging"""
        try:
            recent_data = dataframe.tail(50)
            fisher = self.calculate_fisher_simple(recent_data, params["fisher_period"])
            baseline = ema(recent_data["close"], length=params["baseline_period"])
            ...
            for i in range(len(recent_data)):
                ...
                strategy_returns.append(
                    position * returns.iloc[i] if not pd.isna(returns.iloc[i]) else 0
                )
            ...
            if len(predictions) > 0 and len(predictions[0]) > 0:
                ...
                for i, name in enumerate(param_names):
                    if i < len(model_predictions):
                        ...
                        param_dict[name] = self._generate_derived_parameter(
                            name, param_dict
                        )

            if param_dict:
                EnhancedLogger.log_success(
                    f"ML predicted {len(param_dict)} parameters", "🔮"
                )
                # Log which were direct predictions vs derived
                direct_count = min(
                    len(param_names), len(predictions[0]) if len(predictions) > 0 else 0
                )
                derived_count = len(param_dict) - direct_count
                if derived_count > 0:
                    EnhancedLogger.log_warning(
                        f"Derived {derived_count} parameters from ML base", "🔄"
                    )
            else:
    ...
    def _generate_derived_parameter(
        self, param_name: str, existing_params: dict
    ) -> float:
    ...
    def update_model(self, dataframe: DataFrame, performance: float):
        ...
        try:
            ...
            if len(self.performance_history) >= 20:
                ...
                EnhancedLogger.log_success(
                    f"ML Model updated with {len(self.performance_history)} samples",
                    "🧠",
                )
                ...
                if hasattr(self.model, "feature_importances_"):
                    ...
                    for i, feat_idx in enumerate(reversed(top_features)):
                        ...
                        EnhancedLogger.log_parameter(
                            f"Feature #{feat_idx}", f"{importance:.3f}", "⭐"
                        )
                ...
                EnhancedLogger.log_warning(
                    f"Need {samples_needed} more samples to train model", "📊"
                )
...
class GKD_FisherTransformV4_ML(IStrategy):
    ...
    def __init__(self, config: dict = None):
        ...
        logger.info(
            "🎯 Startup training will begin immediately upon first data analysis"
        )
        ...
        EnhancedLogger.log_parameter(
            "ML Optimization", self.enable_ml_optimization, "🤖"
        )
    ...
    fisher_period = IntParameter(
        10, 15, default=buy_params.get("fisher_period"), space="buy", optimize=True
    )
    fisher_smooth_long = IntParameter(
        3, 10, default=buy_params.get("fisher_smooth_long"), space="buy", optimize=True
    )
    fisher_smooth_short = IntParameter(
        3,
        10,
        default=buy_params.get("fisher_smooth_short"),
        space="buy",
        optimize=can_short,
    )
    fisher_short_exit = DecimalParameter(
        -1.0,
        1.0,
        default=sell_params.get("fisher_short_exit"),
        decimals=3,
        space="sell",
        optimize=can_short,
    )
    fisher_long_exit = DecimalParameter(
        -1.0,
        1.0,
        default=sell_params.get("fisher_long_exit"),
        decimals=3,
        space="sell",
        optimize=True,
    )
    fisher_sell_threshold = DecimalParameter(
        2.0,
        3.9,
        default=sell_params.get("fisher_sell_threshold"),
        decimals=2,
        space="sell",
        optimize=False,
    )
    fisher_buy_threshold = DecimalParameter(
        -1.0,
        2.5,
        default=buy_params.get("fisher_buy_threshold"),
        decimals=2,
        space="buy",
        optimize=True,
    )
    baseline_period = IntParameter(
        5, 21, default=buy_params.get("baseline_period"), space="buy", optimize=True
    )
    atr_period = IntParameter(
        7, 21, default=buy_params.get("atr_period"), space="buy", optimize=True
    )
    goldie_locks = DecimalParameter(
        1.5,
        3.0,
        default=buy_params.get("goldie_locks"),
        decimals=2,
        space="buy",
        optimize=True,
    )
    ...
    ml_confidence_threshold = DecimalParameter(
        0.5, 0.9, default=0.7, decimals=2, space="buy", optimize=True
    )
    ml_adaptation_rate = DecimalParameter(
        0.1, 0.5, default=0.2, decimals=2, space="buy", optimize=True
    )

    # Risk management with ML
    ATR_SL_short_Multip = DecimalParameter(
        1.0, 6.0, decimals=1, default=1.5, space="sell", optimize=True
    )
    ATR_SL_long_Multip = DecimalParameter(
        1.0, 6.0, decimals=1, default=1.5, space="sell", optimize=True
    )
    ATR_Multip = DecimalParameter(
        1.0, 6.0, decimals=1, default=1.5, space="sell", optimize=True
    )
    rr_long = DecimalParameter(
        1.0, 4.0, decimals=1, default=4.0, space="sell", optimize=True
    )
    rr_short = DecimalParameter(
        1.0, 4.0, decimals=1, default=4.0, space="sell", optimize=True
    )
    ...
    def get_ml_adjusted_params(self, dataframe: DataFrame, pair: str) -> dict:
        ...
        try:
            ...
            if not self.enable_ml_optimization or len(dataframe) < 50:
                EnhancedLogger.log_warning(
                    f"ML optimization skipped for {pair} (disabled or insufficient data)",
                    "⚠️",
                )
                return {}
            ...
            if pair not in self.ml_optimizers:
                self.ml_optimizers[pair] = MLOptimizer(
                    f"fisher_transform_v4_{pair.replace('/', '_')}"
                )
            ...
            should_update = (
                self.last_ml_update is None
                or (current_time - self.last_ml_update).total_seconds()
                > self.ml_update_frequency * 3600
            )
            ...
            if should_update and len(dataframe) > 100:
                try:
                    ...
                    EnhancedLogger.log_performance(
                    ...
                    if optimized_params:
                        ...
                        optimized_params["score"] = (
                            ml_optimizer.study.best_value if ml_optimizer.study else 0.0
                        )

                        EnhancedLogger.log_success(
                            f"Optuna returned {len(optimized_params)} parameters", "🎯"
                        )
                        ...
                        EnhancedLogger.log_warning(
                            f"Optuna optimization returned empty results for {pair}",
                            "⚠️",
                        )

                except Exception as e:
                    EnhancedLogger.log_error(
                        f"ML optimization error for {pair}: {str(e)}", "❌"
                    )

            # Get ML predictions for optimal parameters for this pair (fallback)
            try:
                EnhancedLogger.log_subsection(
                    f"Getting ML predictions for {pair}", "🔮"
                )
                ...
                if ml_params:
                    EnhancedLogger.log_success(
                        f"Using ML predicted parameters: {len(ml_params)} total", "✨"
                    )
                ...
                EnhancedLogger.log_error(
                    f"ML prediction error for {pair}: {str(e)}", "❌"
                )
    ...
    def _ensure_all_parameters(self, params: dict, pair: str) -> None:
        ...
        if missing_count > 0:
            EnhancedLogger.log_warning(
                f"Added {missing_count} missing parameters for {pair}", "🔧"
            )
            ...
            EnhancedLogger.log_parameter(
                ...
                (
                    ...
                    else ", ".join(added_params)
                ),
                "📋",
            )
        else:
            EnhancedLogger.log_success(
                f"All {len(required_params)} parameters verified for {pair}", "✅"
            )
    ...
    def calculate_recent_performance(self, pair: str = None) -> float:
        ...
        try:
            if pair:
                pair_trades = [
                    perf
                    for p, perf in self.trade_performance_cache.items()
                    if p == pair
                ]
                if not pair_trades:
                ...
                EnhancedLogger.log_performance(
                    f"Recent Performance ({pair})", performance, "🎯"
                )
                return performance
            else:
                ...
                EnhancedLogger.log_performance(
                    "Overall Recent Performance", performance, "🌟"
                )
                return performance
        except Exception as e:
    ...
    def log_formatted_parameters(self, pair: str, params: Dict[str, Any]):
        ...
        for param, (emoji, name) in exit_params.items():
            if param in params:
                ...
                if isinstance(value, (int, float)):
                    ...
                    EnhancedLogger.log_parameter(
                        f"{name} {color_status}", f"{value:.3f}", emoji
                    )
                else:
                ...
                if isinstance(value, (int, float)):
                    ...
                    if "sl_" in param:  # Stop loss multipliers
                        risk_level = (
                            "🟢 CONSERVATIVE"
                            if value <= 2.0
                            else "🟡 MODERATE" if value <= 4.0 else "🔴 AGGRESSIVE"
                        )
                        EnhancedLogger.log_parameter(
                            f"{name} ({risk_level})", f"{value:.2f}x", emoji
                        )
                    else:  # Risk/Reward ratios
                        rr_quality = (
                            "🟢 EXCELLENT"
                            if value >= 3.0
                            else "🟡 GOOD" if value >= 2.0 else "🔴 RISKY"
                        )
                        EnhancedLogger.log_parameter(
                            f"{name} ({rr_quality})", f"{value:.1f}:1", emoji
                        )
                else:
                    EnhancedLogger.log_parameter(name, value, emoji)
        ...
        EnhancedLogger.log_parameter(
            "Optimization Time", datetime.datetime.now().strftime("%H:%M:%S"), "⏰"
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        if (
            self.startup_training_enabled
            and pair not in self.initial_training_completed
            and len(dataframe) >= self.startup_training_periods
        ):
        ...
        if ml_params:
            EnhancedLogger.log_subsection(
                "ML parameters detected, recalculating...", "🔄"
            )
            ...
            fisher_smooth_short = ml_params.get(
                "fisher_smooth_short", fisher_smooth_short
            )
            ...
            for param_name in original_params:
                ...
                if old_val != new_val:
                    changes_made += 1
                    EnhancedLogger.log_parameter(
                        f"{param_name} changed", f"{old_val} → {new_val}", "🔄"
                    )

            if changes_made > 0:
                EnhancedLogger.log_success(
                    f"Applied {changes_made} ML parameter adjustments", "🎯"
                )

                # Recalculate indicators with ML-adjusted parameters
                if fisher_period != self.fisher_period.value:
                    dataframe["fisher"] = self.calculate_fisher(
                        dataframe, fisher_period
                    )
                    EnhancedLogger.log_success(
                        "Fisher recalculated with ML params", "🔄"
                    )

                if baseline_period != self.baseline_period.value:
                    dataframe["baseline"] = ema(
                        dataframe["close"], length=baseline_period
                    )
                    dataframe["baseline_diff"] = dataframe["baseline"].diff()
                    EnhancedLogger.log_success(
                        "Baseline recalculated with ML params", "🔄"
                    )
            ...
            EnhancedLogger.log_warning(
                "No ML parameters available, using defaults", "⚠️"
            )
        ...
        try:
            ...
            dataframe["fisher_smooth_long"] = ema(
                dataframe["fisher"], length=fisher_smooth_long
            )
            dataframe["fisher_smooth_short"] = ema(
                dataframe["fisher"], length=fisher_smooth_short
            )
            dataframe["fisher_trend_long"] = ema(
                dataframe["fisher_smooth_short"], length=21
            )
            dataframe["fisher_trend_short"] = ema(
                dataframe["fisher_smooth_short"], length=21
            )
            ...
            EnhancedLogger.log_parameter(
                "Baseline Uptrend %", f"{trend_up_pct:.1f}%", "📈"
            )

            # Volatility (ATR for Goldie Locks Zone)
            dataframe["goldie_min"] = dataframe["baseline"] - (
                dataframe["atr"] * self.goldie_locks.value
            )
            dataframe["goldie_max"] = dataframe["baseline"] + (
                dataframe["atr"] * self.goldie_locks.value
            )
            ...
            regime_text = (
                "🐂 BULL"
                if current_regime > 0
                else "🐻 BEAR" if current_regime < 0 else "⚖️ NEUTRAL"
            )

            EnhancedLogger.log_section("ML INDICATOR SUMMARY", "🤖")
            EnhancedLogger.log_parameter(
                "Avg ML Confidence", f"{avg_confidence:.1%}", "🎯"
            )
            EnhancedLogger.log_parameter(
                "Avg Signal Strength", f"{avg_signal_strength:.3f}", "⚡"
            )
        ...
        if ml_params:
            EnhancedLogger.log_success(
                f"ML parameters active: {len(ml_params)} adjustments", "🎯"
            )
            EnhancedLogger.log_parameter(
                "Active optimizers", len(self.ml_optimizers), "🤖"
            )
    ...
    def perform_startup_training(self, dataframe: DataFrame, pair: str):
        ...
        try:
            logger.info(
                f"🧠 [STARTUP] Training ML model for {pair} with {len(dataframe)} historical candles"
            )
            ...
            if len(training_data) > 0:
                logger.info(
                    f"📈 [STARTUP] Generated {len(training_data)} training samples for {pair}"
                )
                ...
                if len(ml_optimizer.performance_history) >= 20:
                    ml_optimizer.update_model(
                        dataframe, 0.0
                    )  # Use neutral performance for initial training
                    logger.info(
                        f"✅ [STARTUP] ML model trained successfully for {pair}"
                    )

                    # Run initial Optuna optimization
                    logger.info(
                        f"🎯 [STARTUP] Running initial parameter optimization for {pair}"
                    )
                    optimized_params = ml_optimizer.optimize_parameters(dataframe, 0.0)

                    if optimized_params:
                        logger.info(
                            f"🎉 [STARTUP] Initial optimization complete for {pair}"
                        )
                        self.log_formatted_parameters(pair, optimized_params)
                    else:
                        logger.warning(
                            f"⚠️ [STARTUP] Initial optimization failed for {pair}"
                        )
                else:
                    logger.warning(
                        f"⚠️ [STARTUP] Insufficient training data generated for {pair}"
                    )
            else:
                logger.error(
                    f"❌ [STARTUP] Failed to generate training data for {pair}"
                )

        except Exception as e:
            logger.error(f"❌ [STARTUP] Training failed for {pair}: {str(e)}")

    def generate_historical_training_data(
        ...
        try:
            training_samples = []
            lookback_period = min(
                500, len(dataframe) - 100
            )  # Use up to 500 candles for training

            logger.info(
                f"🔍 [STARTUP] Analyzing {lookback_period} historical periods for {pair}"
            )

            # Calculate basic indicators needed for analysis
            dataframe_copy = dataframe.copy()
            dataframe_copy["atr"] = talib.ATR(
                dataframe_copy["high"],
                dataframe_copy["low"],
                dataframe_copy["close"],
                timeperiod=14,
            )
            ...
            for i in range(
                100, lookback_period
            ):  # Skip first 100 for indicator stability
                try:
                    ...
                    future_performance = self.calculate_future_performance(
                        dataframe_copy, i, periods=15
                    )

                    if (
                        not np.isnan(future_performance)
                        and abs(future_performance) < 0.5
                    ):  # Filter extreme values
                        training_sample = {
                            "features": features,
                            "performance": future_performance,
                            "timestamp": datetime.datetime.now()
                            - datetime.timedelta(hours=lookback_period - i),
                        }
            ...
            logger.info(
                f"📊 [STARTUP] Generated {len(training_samples)} valid training samples for {pair}"
            )
            return training_samples

        except Exception as e:
            logger.error(
                f"❌ [STARTUP] Error generating training data for {pair}: {str(e)}"
            )
            return []
        ...
        try:
            ...
            for period in [5, 10, 20]:
                pct_change = (
                    ...
                    / dataframe["close"].iloc[max(0, index - period)]
                    - 1
                )
                features.append(pct_change if not pd.isna(pct_change) else 0.0)
            ...
            if "volume" in dataframe.columns:
                ...
                vol_pct = (
                    dataframe["volume"].iloc[index]
                    / dataframe["volume"].iloc[max(0, index - 1)]
                    - 1
                    if index > 0
                    else 0.0
                )
            ...
            features.extend(
                [
                    vol_mean if not pd.isna(vol_mean) else 1000.0,
            ...
            baseline_diff_mean = (
                dataframe["baseline_diff"].iloc[max(0, index - 5) : index].mean()
            )
            baseline_diff_sum = (
                dataframe["baseline_diff"].iloc[max(0, index - 10) : index].sum()
            )
    ...
    def calculate_future_performance(
        self, dataframe: DataFrame, index: int, periods: int = 15
    ) -> float:
    ...
    def identify_market_regime(self, dataframe: DataFrame) -> pd.Series:
        ...
        try:
            ...
            EnhancedLogger.log_parameter(
                "Bull Periods", f"{bull_periods}/50 ({bull_periods*2:.0f}%)", "🐂"
            )
            EnhancedLogger.log_parameter(
                "Bear Periods", f"{bear_periods}/50 ({bear_periods*2:.0f}%)", "🐻"
            )
            EnhancedLogger.log_parameter(
                "Neutral Periods",
                f"{neutral_periods}/50 ({neutral_periods*2:.0f}%)",
                "⚖️",
            )
    ...
    def calculate_signal_strength(self, dataframe: DataFrame) -> pd.Series:
        ...
        try:
            ...
            signal_strength = (
                fisher_strength * 0.4 + trend_strength * 0.4 + volume_strength * 0.2
            )
            ...
            EnhancedLogger.log_parameter(
                "Avg Signal Strength", f"{avg_strength:.3f}", "⚡"
            )
            EnhancedLogger.log_parameter(
                "Current Strength", f"{current_strength:.3f}", "📊"
            )
            EnhancedLogger.log_parameter(
                "Strong Signals (>0.7)", f"{strong_signals}/50", "💪"
            )
    ...
    def calculate_fisher(self, dataframe: DataFrame, period: int) -> pd.Series:
        ...
        try:
            EnhancedLogger.log_subsection(
                f"Calculating Fisher Transform (period={period})", "🎣"
            )
            ...
            EnhancedLogger.log_parameter(
                "Current Fisher", f"{current_fisher:.3f}", "🎣"
            )
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        fisher_buy_threshold = ml_params.get(
            "fisher_buy_threshold", self.fisher_buy_threshold.value
        )
        fisher_sell_threshold = ml_params.get(
            "fisher_sell_threshold", self.fisher_sell_threshold.value
        )
        ...
        EnhancedLogger.log_parameter(
            "High Confidence Periods", f"{ml_conf_count}/{len(dataframe)}", "🎯"
        )
        EnhancedLogger.log_parameter(
        ...
        regime_emoji = (
            "🐂" if current_regime > 0 else "🐻" if current_regime < 0 else "⚖️"
        )
        regime_text = (
            "BULL"
            if current_regime > 0
            else "BEAR" if current_regime < 0 else "NEUTRAL"
        )
        EnhancedLogger.log_parameter(
            "Market Regime", f"{regime_text} {regime_emoji}", "🏛️"
        )
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        fisher_long_exit = ml_params.get(
            "fisher_long_exit", self.fisher_long_exit.value
        )
        fisher_short_exit = ml_params.get(
            "fisher_short_exit", self.fisher_short_exit.value
        )

        EnhancedLogger.log_section("EXIT THRESHOLDS", "🎚️")
        exit_color_long = (
            "🟢" if fisher_long_exit > 0 else "🔴" if fisher_long_exit < 0 else "🟡"
        )
        exit_color_short = (
            "🟢" if fisher_short_exit > 0 else "🔴" if fisher_short_exit < 0 else "🟡"
        )

        EnhancedLogger.log_parameter(
            f"Long Exit {exit_color_long}", f"{fisher_long_exit:.3f}", "📤"
        )
        if self.can_short:
            EnhancedLogger.log_parameter(
                f"Short Exit {exit_color_short}", f"{fisher_short_exit:.3f}", "📥"
            )
        ...
        long_exit_conditions = (
            ...
            & (dataframe["fisher_smooth_long"] > dataframe["fisher"])
            & (ml_confidence > 0.5)  # Only exit with reasonable confidence
        )

        dataframe.loc[long_exit_conditions, ["exit_long", "exit_tag"]] = [
            1,
            "exit_long_ml",
        ]
        ...
        if self.can_short:
            ...
            dataframe.loc[short_exit_conditions, ["exit_short", "exit_tag"]] = [
                1,
                "exit_short_ml",
            ]
        ...
        recent_long_exit = (
            dataframe["exit_long"].tail(20).sum()
        ...
        if self.can_short:
            recent_short_exit = (
                dataframe["exit_short"].tail(20).sum()
        ...
        EnhancedLogger.log_parameter(
            "Long Distance to Exit", f"{long_distance_to_exit:.3f}", "📏"
        )
        ...
        if self.can_short:
            ...
            EnhancedLogger.log_parameter(
                "Short Distance to Exit", f"{short_distance_to_exit:.3f}", "📏"
            )
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        EnhancedLogger.log_parameter(
            "Trade Duration", str(current_time - trade.open_date_utc), "⏱️"
        )
        ...
        try:
            ...
            if not dataframe.empty:
                ...
                EnhancedLogger.log_ml_status(
                    "Current ML State", current_ml_confidence, "🤖"
                )

                regime_text = (
                    "BULL 🐂"
                    if market_regime > 0
                    else "BEAR 🐻" if market_regime < 0 else "NEUTRAL ⚖️"
                )
                EnhancedLogger.log_parameter("Market Regime", regime_text, "🏛️")
        ...
        **kwargs,
        ...
        if exit_reason in ["roi", "stop_loss", "ml_stop_loss"]:
            ...
            performance_quality = (
                ...
                else "🟡 BREAK-EVEN" if current_profit > -0.01 else "🔴 LOSS"
            )
            EnhancedLogger.log_parameter(
                f"Performance {performance_quality}", f"{current_profit:.2%}", "📈"
            )
    ...
    def custom_stake_amount(
        ...
        **kwargs,
        ...
        try:
            ...
            EnhancedLogger.log_parameter(
                "Proposed Stake", f"{proposed_stake:.4f}", "💵"
            )
            ...
            if not dataframe.empty:
                ...
                EnhancedLogger.log_parameter(
                    "Confidence Multiplier", f"{confidence_multiplier:.3f}", "🎯"
                )
                EnhancedLogger.log_parameter(
                    "Signal Multiplier", f"{signal_multiplier:.3f}", "⚡"
                )
                EnhancedLogger.log_parameter(
                    "Combined ML Adjustment", f"{ml_adjustment:.3f}x", "🤖"
                )

                if ml_adjustment > 1.0:
                    EnhancedLogger.log_success(
                        "Increasing stake due to strong ML signals", "📈"
                    )
                elif ml_adjustment < 0.8:
                    EnhancedLogger.log_warning(
                        "Reducing stake due to weak ML signals", "📉"
                    )
                else:
                    EnhancedLogger.log_success(
                        "Standard stake with moderate ML adjustment", "⚖️"
                    )
            else:
                EnhancedLogger.log_warning(
                    "No dataframe available, using default adjustment", "⚠️"
                )

            custom_stake = (
                proposed_stake / self.max_so_multiplier * self.overbuy_factor
            ) * ml_adjustment
            custom_stake = max(
                min_stake, min(custom_stake, max_stake)
        ...
        change_emoji = (
            "📈" if stake_change_pct > 0 else "📉" if stake_change_pct < 0 else "➡️"
        )
        EnhancedLogger.log_parameter(
            f"Stake Change {change_emoji}", f"{stake_change_pct:+.1f}%", "📊"
        )
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        EnhancedLogger.log_parameter(
            "Max SO Multiplier", self.max_so_multiplier_orig, "🔢"
        )
        ...
        if 1 <= count_of_buys <= self.max_so_multiplier_orig:
            ...
            try:
                ...
                if not dataframe.empty:
                    ...
                    if ml_confidence < 0.5:
                        ml_trigger_adjustment = (
                            1.5  # More conservative in low confidence
                        )
                        EnhancedLogger.log_warning(
                            "Conservative DCA due to low confidence", "⚠️"
                        )
                    elif market_regime < 0 and not trade.is_short:
                        ml_trigger_adjustment = (
                            1.3  # More conservative for longs in bear market
                        )
                        EnhancedLogger.log_warning(
                            "Conservative DCA for long in bear market", "🐻"
                        )
                    else:
                        EnhancedLogger.log_success("Standard DCA trigger", "✅")

                    EnhancedLogger.log_parameter(
                        "ML Trigger Adjustment", f"{ml_trigger_adjustment:.1f}x", "🎯"
                    )
            ...
            EnhancedLogger.log_parameter(
                "Safety Order Trigger", f"{safety_order_trigger:.2%}", "🎯"
            )
            ...
            if current_profit <= (-1 * abs(safety_order_trigger)):
                ...
                try:
                    ...
                    EnhancedLogger.log_parameter(
                        "Initial Stake", f"{actual_initial_stake:.4f}", "💰"
                    )
                    EnhancedLogger.log_parameter(
                        "Already Invested", f"{already_bought:.4f}", "💸"
                    )

                    if trade.pair in self.cust_proposed_initial_stakes:
                        if self.cust_proposed_initial_stakes[trade.pair] > 0:
                            proposed_initial_stake = self.cust_proposed_initial_stakes[
                                trade.pair
                            ]
                            ...
                            EnhancedLogger.log_parameter(
                                "Compensated Stake", f"{stake_amount:.4f}", "🎯"
                            )
                        else:
                            ...
                            EnhancedLogger.log_parameter(
                                "Scaled Stake", f"{stake_amount:.4f}", "📈"
                            )
                    else:
                        stake_amount = stake_amount * math.pow(
                            self.safety_order_volume_scale, (count_of_buys - 1)
                        ...
                        EnhancedLogger.log_parameter(
                            "Default Scaled Stake", f"{stake_amount:.4f}", "📊"
                        )

                    EnhancedLogger.log_success(
                        f"DCA order #{count_of_buys + 1} approved", "✅"
                    )
                    return stake_amount
                ...
                EnhancedLogger.log_parameter(
                    "Distance to DCA", f"{distance_to_trigger:.2%}", "📏"
                )
            ...
            if not trade_candle.empty:
                ...
                try:
                    ...
                    tp_multiplier = (
                        self.rr_long.value
                        if not trade.is_short
                        else self.rr_short.value
                    )

                    EnhancedLogger.log_parameter(
                        "Base SL Multiplier", f"{sl_multiplier:.1f}x", "🛡️"
                    )
                    EnhancedLogger.log_parameter(
                        "Base TP Multiplier", f"{tp_multiplier:.1f}x", "🎯"
                    )
                    EnhancedLogger.log_ml_status(
                        "ML Confidence", current_ml_confidence, "🤖"
                    )
                    ...
                    if not trade.is_short and market_regime < 0:  # Long in bear market
                        sl_multiplier *= 0.9
                        EnhancedLogger.log_warning(
                            "Tighter SL for long in bear market", "🐻"
                        )
                    elif trade.is_short and market_regime > 0:  # Short in bull market
                        sl_multiplier *= 0.9
                        EnhancedLogger.log_warning(
                            "Tighter SL for short in bull market", "🐂"
                        )

                    EnhancedLogger.log_parameter(
                        "Adjusted SL Multiplier", f"{sl_multiplier:.2f}x", "🎯"
                    )
                    EnhancedLogger.log_parameter(
                        "Adjusted TP Multiplier", f"{tp_multiplier:.2f}x", "🎯"
                    )
                ...
                EnhancedLogger.log_parameter(
                    f"{side_text} SL Price", f"{sl_price:.6f}", "🛑"
                )
                EnhancedLogger.log_parameter(
                ...
                if sl_condition:
                    self.trade_performance_cache[pair] = (
                        current_profit  # Store for ML learning
                    )
                ...
                EnhancedLogger.log_parameter(
                    f"{side_text} TP Price", f"{tp_price:.6f}", "🎯"
                )
                ...
                EnhancedLogger.log_parameter(
                    "Distance to SL", f"{sl_distance_current:.6f}", "📏"
                )
                EnhancedLogger.log_parameter(
                    "Distance to TP", f"{tp_distance_current:.6f}", "📏"
                )
    ...
    def leverage(
        ...
        **kwargs,
        ...
        try:
            ...
            EnhancedLogger.log_parameter(
                "Proposed Leverage", f"{proposed_leverage:.1f}x", "📊"
            )
            ...
            if not dataframe.empty:
                ml_confidence = dataframe["ml_confidence"].iloc[-1]
                market_volatility = (
                    dataframe["atr"].iloc[-1] / dataframe["close"].iloc[-1]
                )

                EnhancedLogger.log_ml_status("ML Confidence", ml_confidence, "🤖")
                EnhancedLogger.log_parameter(
                    "Market Volatility", f"{market_volatility:.1%}", "🌊"
                )
                ...
                if ml_confidence < 0.6 or market_volatility > 0.05:
                    ...
                    reason = (
                        "low confidence" if ml_confidence < 0.6 else "high volatility"
                    )
                    ...
                    EnhancedLogger.log_success(
                        "Increased leverage due to favorable conditions", "📈"
                    )
                else:
                ...
                change_emoji = (
                    "📈"
                    if leverage_change > 0
                    else "📉" if leverage_change < 0 else "➡️"
                )

                EnhancedLogger.log_parameter(
                    "Final Leverage", f"{final_leverage:.1f}x", "🎯"
                )
                EnhancedLogger.log_parameter(
                    f"Leverage Change {change_emoji}", f"{leverage_change:+.1f}x", "📊"
                )
        ...
        EnhancedLogger.log_parameter(
            "Default Leverage Applied", f"{default_leverage:.1f}x", "🔄"
        )
