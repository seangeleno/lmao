...
class UltraSmartStrategy(IStrategy):
    ...
    def bot_start(self, **kwargs) -> None:
        ...
        logger.info(
            f"🎯 交易风格管理系统已启动 - 当前模式: {self.style_manager.current_style}"
        )
...
=================================================="""
    ...
    def calculate_optimized_composite_indicators(
        ...
        ema_spread_series = self._safe_series(
            ema_spread, len(dataframe), index=dataframe.index
        )
        ...
        volume_trend_series = self._safe_series(
            volume_trend, len(dataframe), index=dataframe.index
        )
        ...
        price_velocity_series = self._safe_series(
            price_velocity, len(dataframe), index=dataframe.index
        )
        ...
        new_columns["price_decelerating"] = (
            np.abs(price_acceleration_new) < np.abs(price_acceleration_new.shift(3))
        ).astype(int)
    ...
    def identify_support_resistance(
        ...
        sr_columns = {
            "local_max": dataframe["high"].rolling(window, center=True).max()
            == dataframe["high"],
            "local_min": dataframe["low"].rolling(window, center=True).min()
            == dataframe["low"],
            "resistance_distance": np.where(
                dataframe["close"] > 0,
                (dataframe["high"].rolling(50).max() - dataframe["close"])
                / dataframe["close"],
                0,
            ),
            "support_distance": np.where(
                dataframe["close"] > 0,
    ...
    def calculate_wave_analysis(self, dataframe: DataFrame) -> DataFrame:
        ...
        wave_columns = {
            "wave_strength": abs(dataframe["close"] - dataframe["close"].shift(5))
            / (dataframe["close"].shift(5) + 1e-10),
            "normalized_returns": returns / (returns.rolling(20).std() + 1e-10),
            "momentum_dispersion": dataframe["mom_10"].rolling(10).std()
    ...
    def identify_coin_risk_tier(self, pair: str, dataframe: DataFrame) -> str:
        ...
        try:
            ...
            # === 特征1: 价格波动率分析 ===
            volatility = (
                dataframe["atr_p"].iloc[current_idx]
                if "atr_p" in dataframe.columns
                else 0.05
            )
            volatility_24h = (
                dataframe["close"].rolling(96).std().iloc[current_idx]
                / dataframe["close"].iloc[current_idx]
            )
            ...
            price_24h_ago = (
                dataframe["close"].iloc[-96]
                if len(dataframe) >= 96
                else dataframe["close"].iloc[0]
            )
            price_change_24h = (
                abs((current_price / price_24h_ago) - 1) if price_24h_ago > 0 else 0
            )
            ...
            # === 特征5: 技术指标异常检测 ===
            rsi = (
                dataframe["rsi_14"].iloc[current_idx]
                if "rsi_14" in dataframe.columns
                else 50
            )
...
"""
    ...
    def calculate_dynamic_position_size(
        ...
        if quality_column in dataframe.columns and len(dataframe) > 0:
            ...
            current_grade = (
                dataframe[grade_column].iloc[-1]
                if grade_column in dataframe.columns
                else "C"
            )
        ...
        market_environment_multiplier = (
            mtf_multiplier * noise_multiplier * volatility_multiplier
        ) ** (1 / 3)
        ...
        if (
            volatility_multiplier <= 0.6
            and noise_multiplier <= 0.6
            and signal_quality_multiplier <= 0.8
        ):
    ...
    def _assess_position_risk_level(
        self, position_size: float, max_position: float
    ) -> str:
        """评估仓位风险等级"""
...
"""
            ...
            # 降级到原始系统
            return self._fallback_position_calculation(
                current_price, market_state, pair
            )

    def _fallback_position_calculation(
        self, current_price: float, market_state: str, pair: str
    ) -> float:
        """备用仓位计算系统（当增强系统失败时使用）"""
...
"""
    ...
    def _log_trade_entry_targets(
        self, pair: str, entry_price: float, leverage_params: dict
    ):
        """
        📊 记录详细的交易入场目标价格
        清晰显示所有预计算的价格级别
...
"""
    ...
    def calculate_leverage_adjusted_params(
        self,
        leverage: int,
        atr_value: float,
        entry_price: float,
        ...
        if not is_short:
            ...
            distance_to_liquidation = (
                (entry_price - liquidation_price) / entry_price * 100
            )
        else:
            distance_to_stop = (stop_loss_price - entry_price) / entry_price * 100
            distance_to_liquidation = (
                (liquidation_price - entry_price) / entry_price * 100
            )
    ...
    def validate_and_calibrate_indicators(self, dataframe: DataFrame) -> DataFrame:
        ...
        try:
            ...
            if "rsi_14" in dataframe.columns:
                ...
                logger.info(
                    f"RSI校准完成 - 原始空值: {original_rsi_nulls}, 范围限制: 0-100"
                )
            ...
            for ema_col in ["ema_8", "ema_21", "ema_50"]:
                if ema_col in dataframe.columns:
                    ...
                    if "close" in dataframe.columns:
                        ...
                        if outliers > 0:
                            logger.warning(
                                f"{ema_col} 发现 {outliers} 个异常值，重新计算"
                            )
                            # 重新计算该EMA
                            if ema_col == "ema_8":
                                dataframe[ema_col] = ta.EMA(dataframe, timeperiod=8)
...
"""
            ...
            for col in price_cols:
                if col in dataframe.columns:
                    ...
                    if price_std / price_mean < 0.001:  # 变化率低于0.1%
                        logger.warning(
                            f"价格数据变化异常小 {pair}: {col} std/mean = {price_std/price_mean:.6f}"
                        )
            ...
            if "volume" in dataframe.columns:
                if dataframe["volume"].sum() == 0:
                    ...
                    if volume_mean > 0 and volume_std / volume_mean < 0.1:
                        logger.warning(
                            f"成交量数据变化异常小 {pair}: std/mean = {volume_std/volume_mean:.6f}"
                        )
            ...
            if "date" in dataframe.columns or dataframe.index.name == "date":
                ...
                if len(time_diff) > 0:
                    ...
                    if (
                        ...
                        logger.warning(
                            f"时间间隔异常 {pair}: {abnormal_intervals}/{len(time_diff)} 个异常间隔 (预期间隔: {expected_interval})"
                        )
    ...
    def analyze_multi_timeframe(self, dataframe: DataFrame, metadata: dict) -> Dict:
        ...
        timeframes = {
            ...
            },  # 主/中周期（与策略timeframe一致则使用传入dataframe）
        ...
        for tf, config in timeframes.items():
            try:
                ...
                if "rsi_14" not in tf_dataframe.columns:
                    tf_dataframe["rsi_14"] = ta.RSI(
                        tf_dataframe["close"], timeperiod=14
                    )
                if "ema_21" not in tf_dataframe.columns:
                    tf_dataframe["ema_21"] = ta.EMA(
                        tf_dataframe["close"], timeperiod=21
                    )
                if "ema_50" not in tf_dataframe.columns:
                    tf_dataframe["ema_50"] = ta.EMA(
                        tf_dataframe["close"], timeperiod=50
                    )
                if "adx" not in tf_dataframe.columns:
                    tf_dataframe["adx"] = ta.ADX(
                        tf_dataframe["high"],
                        tf_dataframe["low"],
                        tf_dataframe["close"],
                        timeperiod=14,
                ...
                price_momentum = (
                    (
                        (close - tf_dataframe["close"].shift(5).iloc[-1])
                        / tf_dataframe["close"].shift(5).iloc[-1]
                        * 100
                    )
                    if len(tf_dataframe) > 5
                    else 0
                )

                # RSI动量
                rsi_momentum = (
                    rsi - tf_dataframe["rsi_14"].shift(3).iloc[-1]
                    if len(tf_dataframe) > 3
                    else 0
                )
                ...
                confirmation_strength = sum(confirmation_factors) / len(
                    confirmation_factors
                )
                ...
                mtf_analysis[tf] = {
                    ...
                    "macd_trend": (
                        "bullish"
                        if macd_bullish
                        else "bearish" if macd_bearish else "neutral"
                    ),
                }

                logger.debug(
                    f"MTF {tf}: {trend_direction} (强度:{adx_strength}, 位置:{price_position:.2f})"
                )
        ...
        if len(mtf_analysis) >= 2:
            mtf_analysis["mtf_consensus"] = self._calculate_mtf_consensus(
    ...
    def _calculate_mtf_consensus(self, mtf_analysis: Dict, timeframes: Dict) -> Dict:
        ...
        return {
            ...
            "alignment_ratio": (
                max(bullish_ratio, bearish_ratio) if total_tfs > 0 else 0
            ),
        }
    ...
    def _safe_get_bool_series(
        self, dataframe: DataFrame, column: str, default_value: bool = False
    ) -> pd.Series:
        """安全获取布尔系列，确保索引对齐"""
        try:
    ...
    def _safe_get_numeric_series(
        self, dataframe: DataFrame, column: str, default_value: float = 0.0
    ) -> pd.Series:
        """安全获取数值系列，确保索引对齐"""
        try:
            result = dataframe.get(column, default_value)
            if isinstance(result, pd.Series):
                ...
                # 如果返回的是标量，创建对应长度的Series
                return pd.Series(
                    [float(result)] * len(dataframe), index=dataframe.index
                )
        except:
            # 如果出现任何错误，返回默认值Series
            return pd.Series([default_value] * len(dataframe), index=dataframe.index)
    ...
    def calculate_dynamic_rsi_thresholds(
        ...
        # 3. 市场环境调整
        market_sentiment = self._safe_get_numeric_series(
            dataframe, "market_sentiment", 0
        )
        ...
        sentiment_adjustment = np.where(
            extreme_fear_mask,
            -5,  # 极度恐慌时降低超卖阈值
            np.where(extreme_greed_mask, 5, 0),
        )  # 极度贪婪时提高超买阈值
    ...
    def validate_ema_cross_signals(self, dataframe: DataFrame) -> Dict[str, pd.Series]:
        ...
        # === 基础EMA交叉检测 ===
        # 金叉：EMA8上穿EMA21
        basic_golden_cross = (dataframe["ema_8"] > dataframe["ema_21"]) & (
            dataframe["ema_8"].shift(1) <= dataframe["ema_21"].shift(1)
        )

        # 死叉：EMA8下穿EMA21
        basic_death_cross = (dataframe["ema_8"] < dataframe["ema_21"]) & (
            dataframe["ema_8"].shift(1) >= dataframe["ema_21"].shift(1)
        )
        ...
        position_safe_bullish = (price_position > 0.15) & (
            price_position < 0.85
        )  # 不在极端位置
        ...
        # 强信号：至少5个确认条件 (成功率80%+)
        strong_golden_cross = (
            basic_golden_cross & (bullish_score >= 5) & bullish_continuation
        )
        strong_death_cross = (
            basic_death_cross & (bearish_score >= 5) & bearish_continuation
        )

        # 中等信号：至少3个确认条件 (成功率65%+)
        medium_golden_cross = (
            basic_golden_cross & (bullish_score >= 3) & bullish_continuation
        )
        medium_death_cross = (
            basic_death_cross & (bearish_score >= 3) & bearish_continuation
        )
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        try:
            ...
            dataframe["rsi_overbought_extreme"] = dynamic_rsi_thresholds[
                "overbought_extreme"
            ]
            dataframe["rsi_oversold_extreme"] = dynamic_rsi_thresholds[
                "oversold_extreme"
            ]
        ...
        try:
            ...
            # 将验证结果添加到dataframe中
            dataframe["ema_strong_golden_cross"] = ema_cross_validation[
                "strong_golden_cross"
            ]
            dataframe["ema_strong_death_cross"] = ema_cross_validation[
                "strong_death_cross"
            ]
            dataframe["ema_medium_golden_cross"] = ema_cross_validation[
                "medium_golden_cross"
            ]
            dataframe["ema_medium_death_cross"] = ema_cross_validation[
                "medium_death_cross"
            ]
            dataframe["ema_bullish_score"] = ema_cross_validation[
                "bullish_confirmation_score"
            ]
            dataframe["ema_bearish_score"] = ema_cross_validation[
                "bearish_confirmation_score"
            ]

            # 统计信号质量
            strong_signals = (
                ema_cross_validation["strong_golden_cross"].sum()
                + ema_cross_validation["strong_death_cross"].sum()
            )
            weak_signals = (
                ema_cross_validation["weak_golden_cross"].sum()
                + ema_cross_validation["weak_death_cross"].sum()
            )
            ...
            # 降级处理：使用基础EMA交叉
            dataframe["ema_strong_golden_cross"] = (
                dataframe["ema_8"] > dataframe["ema_21"]
            ) & (dataframe["ema_8"].shift(1) <= dataframe["ema_21"].shift(1))
            dataframe["ema_strong_death_cross"] = (
                dataframe["ema_8"] < dataframe["ema_21"]
            ) & (dataframe["ema_8"].shift(1) >= dataframe["ema_21"].shift(1))
    ...
    def apply_mtf_analysis_to_dataframe(
        ...
        mtf_columns = {
            # === 核心MTF评分 ===
            "mtf_trend_score": self._safe_series(
                mtf_trend_score, len(dataframe)
            ),  # -1 to 1
            "mtf_strength_score": self._safe_series(
                mtf_strength_score, len(dataframe)
            ),  # 0 to 1
            "mtf_risk_score": self._safe_series(
                mtf_risk_score, len(dataframe)
            ),  # -1 to 1
            "mtf_confirmation_score": self._safe_series(
                mtf_confirmation_score, len(dataframe)
            ),  # 0 to 1
            # === MTF共识信息 ===
            "mtf_consensus_direction": self._safe_series(
                consensus_direction, len(dataframe)
            ),
            "mtf_consensus_strength": self._safe_series(
                consensus_strength, len(dataframe)
            ),
            "mtf_alignment_ratio": self._safe_series(alignment_ratio, len(dataframe)),
            "mtf_momentum_consistency": self._safe_series(
                momentum_consistency, len(dataframe)
            ),
            # === MTF信号过滤器（新版本）===
            "mtf_long_filter": self._safe_series(
                1 if mtf_long_filter else 0, len(dataframe)
            ),
            "mtf_short_filter": self._safe_series(
                1 if mtf_short_filter else 0, len(dataframe)
            ),
            # === MTF强信号（严格条件）===
            "mtf_strong_bull": self._safe_series(
                1 if mtf_strong_bull else 0, len(dataframe)
            ),
            "mtf_strong_bear": self._safe_series(
                1 if mtf_strong_bear else 0, len(dataframe)
            ),
            # === MTF等级信号 ===
            "mtf_very_strong_bull": self._safe_series(
                1 if mtf_very_strong_bull else 0, len(dataframe)
            ),
            "mtf_very_strong_bear": self._safe_series(
                1 if mtf_very_strong_bear else 0, len(dataframe)
            ),
            "mtf_moderate_bull": self._safe_series(
                1 if mtf_moderate_bull else 0, len(dataframe)
            ),
            "mtf_moderate_bear": self._safe_series(
                1 if mtf_moderate_bear else 0, len(dataframe)
            ),
            # === 关键价格位（增强版）===
...
"""
    ...
    def _calculate_position_weight(self, dataframe: DataFrame) -> pd.Series:
        ...
        if "signal_quality_score" in dataframe.columns:
            quality_multiplier = (
                0.5 + dataframe["signal_quality_score"] * 1.5
            )  # 0.5-2.0倍
    ...
    def _calculate_leverage_multiplier(self, dataframe: DataFrame) -> pd.Series:
        ...
        if "signal_quality_score" in dataframe.columns:
            # 高质量信号可以使用更高杠杆
            quality_leverage = (
                1.0 + dataframe["signal_quality_score"] * 2.0
            )  # 1.0-3.0倍
        ...
        return (
            base_leverage.fillna(1.0).clip(0.5, 5.0).round().astype(int)
        )  # 1-5倍整数杠杆
    ...
    def filter_5min_noise(self, dataframe: DataFrame) -> Dict[str, pd.Series]:
        ...
        # 异常价格波动（通常是噪音）
        abnormal_price_volatility = price_volatility > price_volatility.rolling(
            20
        ).quantile(0.85)
        ...
        # 快速反转模式（上涨后立即下跌）
        false_breakout_up = (price_change_3min > 0.005) & (  # 3根K线涨幅>0.5%
            price_change_1min < -0.002
        )  # 最后1根K线跌幅>0.2%

        false_breakout_down = (price_change_3min < -0.005) & (  # 3根K线跌幅>0.5%
            price_change_1min > 0.002
        )  # 最后1根K线涨幅>0.2%
        ...
        low_volatility_high_volume = (price_range < 0.003) & (  # 价格范围<0.3%
        ...
        single_candle_spike = (
            ...
            abs(dataframe["close"] - dataframe["open"]) / dataframe["open"] < 0.002
        )  # 但收盘价变化<0.2%
        ...
        atr_14 = (
            ...
            .rolling(14)
            .apply(lambda x: (x.max() - x.min()) / 14)
        )
        price_change_normalized = (
            abs(dataframe["close"] - dataframe["close"].shift(1)) / atr_14
        )
        ...
        # 短期趋势与中期趋势方向相反时可能是噪音
        trend_inconsistency = (short_trend > 0) & (medium_trend < 0) | (
            short_trend < 0
        ) & (medium_trend > 0)
        ...
        return {
            ...
            "avoid_trading": high_noise_zone
            | (abnormal_price_volatility & insufficient_volume),
        }

    def optimize_macd_leading_signals(
        self, dataframe: DataFrame
    ) -> Dict[str, pd.Series]:
        ...
        fast_macd_arr = ta.MACD(
            dataframe["close"], fastperiod=8, slowperiod=17, signalperiod=5
        )
        ...
        if "macd" in dataframe.columns:
            ...
            std_macd_signal = dataframe.get(
                "macd_signal",
                pd.Series(ta.MACD(dataframe["close"])[1], index=dataframe.index),
            )
            std_macd_hist = dataframe.get(
                "macd_hist",
                pd.Series(ta.MACD(dataframe["close"])[2], index=dataframe.index),
            )
        else:
        ...
        slow_macd_arr = ta.MACD(
            dataframe["close"], fastperiod=19, slowperiod=39, signalperiod=9
        )
        ...
        strong_convergence = (
            ...
            & (
                macd_distance < macd_distance.rolling(5).mean() * 0.5
            )  # 距离小于5期平均的50%
        )
        ...
        macd_approaching_bullish = (
            ...
            & (
                abs(std_macd - std_macd_signal)
                < abs(std_macd.shift(2) - std_macd_signal.shift(2)) * 0.3
            )  # 距离大幅缩小
        )
        ...
        macd_near_zero_bullish = (
            (std_macd < 0)
            & (std_macd > -abs(std_macd.rolling(20).std()))  # 接近零轴
        ...
        # === 6. 成交量加权MACD ===
        # 使用成交量来验证MACD信号的有效性
        volume_ratio = dataframe.get(
            "volume_ratio", pd.Series(1, index=dataframe.index)
        )
        volume_weighted_strength = (volume_ratio > 1.2) & (  # 成交量放大
            volume_ratio > volume_ratio.shift(1)
        )  # 成交量递增

        # === 7. 多时间框架MACD确认 ===
        # 使用快速MACD提供早期信号，标准MACD确认
        fast_bullish_cross = (fast_macd_line > fast_macd_signal) & (
            fast_macd_line.shift(1) <= fast_macd_signal.shift(1)
        )

        fast_bearish_cross = (fast_macd_line < fast_macd_signal) & (
            fast_macd_line.shift(1) >= fast_macd_signal.shift(1)
        )
        ...
        volume_confirmed_bullish = (
            bullish_signal_strength * volume_weighted_strength.astype(int)
        )
        volume_confirmed_bearish = (
            bearish_signal_strength * volume_weighted_strength.astype(int)
        )

        # === 最终前置信号（高质量） ===
        # 需要信号强度>0.5且有成交量确认
        leading_macd_bullish = (bullish_signal_strength > 0.5) | (  # 信号强度足够
            volume_confirmed_bullish > 0.3
        )  # 或成交量确认较强

        leading_macd_bearish = (bearish_signal_strength > 0.5) | (  # 信号强度足够
            volume_confirmed_bearish > 0.3
        )  # 或成交量确认较强
    ...
    def calculate_comprehensive_signal_quality(
        self, dataframe: DataFrame, signal_direction: str = "long"
    ) -> Dict[str, pd.Series]:
        """🎯 综合信号质量评分系统 - 整合所有增强功能"""
        ...
        # RSI动态阈值对齐
        if (
            "rsi_dynamic_oversold" in dataframe.columns
            and "rsi_dynamic_overbought" in dataframe.columns
        ):
            if signal_direction == "long":
                rsi_alignment = (
                    (dataframe["rsi_14"] < dataframe["rsi_dynamic_overbought"])
                    & (
                        dataframe["rsi_14"] > dataframe["rsi_dynamic_oversold"] * 0.8
                    )  # 接近但未超买
                ).astype(float) * 100
            else:
                rsi_alignment = (
                    ...
                    & (
                        dataframe["rsi_14"] < dataframe["rsi_dynamic_overbought"] * 1.2
                    )  # 接近但未超卖
                ).astype(float) * 100
        ...
        if "ema_bullish_score" in dataframe.columns:
            if signal_direction == "long":
                ema_alignment = (
                    dataframe["ema_bullish_score"] / 7.0
                ) * 100  # 标准化到0-100
            else:
        ...
        # MACD前置确认对齐
        if (
            "macd_bullish_strength" in dataframe.columns
            and "macd_bearish_strength" in dataframe.columns
        ):
            if signal_direction == "long":
        ...
        # ADX趋势确认
        if "adx" in dataframe.columns:
            adx_quality = np.minimum(
                dataframe["adx"] / 50 * 100, 100
            )  # ADX越高质量越好
        ...
        if alignment_factors:
            ...
            technical_alignment = (
                sum(f * w for f, w in zip(alignment_factors, alignment_weights))
                / weights_sum
            )
        else:
        ...
        if "volume_ratio" in dataframe.columns:
            ...
            volume_optimal = np.where(
                ...
                np.where(
                    ...
                    30,  # 过低
                    np.minimum(
                        (dataframe["volume_ratio"] - 0.8) / 1.7 * 100, 100
                    ),  # 0.8-2.5范围线性映射
                ),
            )
        ...
        if "momentum_score" in dataframe.columns:
            if signal_direction == "long":
                momentum_strength = (
                    np.maximum(dataframe["momentum_score"], 0) * 50
                )  # 标准化
            else:
        ...
        if "close" in dataframe.columns:
            price_momentum_3 = (
                dataframe["close"] / dataframe["close"].shift(3) - 1
            ) * 100
            if signal_direction == "long":
                momentum_consistency = (
                    np.maximum(price_momentum_3, 0) * 10
                )  # 放大到0-100范围
            else:
        ...
        # 加权平均总质量分数
        total_score = sum(
            score * weight for score, weight in quality_components.values()
        )

        # === 8. 质量等级分类 ===
        quality_grade = pd.Series("C", index=dataframe.index)  # 默认C级
        quality_grade = np.where(
            total_score >= 85,
            "A+",
            np.where(
                total_score >= 80,
            ...
            np.where(
                ...
                np.where(
                    ...
                    np.where(
                        total_score >= 60,
                        0.8,  # B-级: 0.8倍
                        np.where(total_score >= 50, 0.5, 0.3),  # C级: 0.5倍
                    ),
                ),
            ),
        )  # D级及以下: 0.3倍
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        rsi_momentum_strong = (
            ...
            & (dataframe["rsi_14"] < 80)
            & (dataframe["rsi_14"] > 20)  # 放宽RSI极值区范围
        )
        ...
        no_fake_breakout = ~(
            ...
            (
                (dataframe["high"] - dataframe["close"])
                > (dataframe["close"] - dataframe["open"]) * 3
            )  # 提高到3倍
            | (
        ...
        long_favourable_environment = (
            ...
            & (
                self._safe_get_numeric_series(dataframe, "market_sentiment", 0) > -0.8
            )  # 情绪不过度悲观（放宽）
        ...
        dynamic_oversold = self._safe_get_numeric_series(
            dataframe, "rsi_oversold_dynamic", 25
        )
        dynamic_overbought = self._safe_get_numeric_series(
            dataframe, "rsi_overbought_dynamic", 75
        )
        ...
        # === 趋势确认：只在上升趋势或横盘中做多 ===
        trend_confirmation = (dataframe["ema_8"] >= dataframe["ema_21"]) | (  # 多头排列
            dataframe["adx"] < 25
        )  # 或横盘环境
        ...
        # === 强度确认：ADX显示趋势开始形成 ===
        strength_confirmation = (dataframe["adx"] > 20) & (  # 最低强度要求
            dataframe["adx"] > dataframe["adx"].shift(2)
        )  # ADX上升

        # === 背离检测：避免在顶背离时入场 ===
        no_bearish_divergence = ~self._safe_get_bool_series(
        ...
        medium_with_extra_confirm = (
            ...
            & (
                self._safe_get_numeric_series(dataframe, "momentum_exhaustion_score", 0)
                < 0.5
            )  # 动量未衰竭
            & (
                ~self._safe_get_bool_series(dataframe, "bearish_divergence", False)
            )  # 无顶背离
        )
        ...
        price_in_top_20pct = (
            (
                dataframe["close"] > dataframe["high"].rolling(50).quantile(0.8)
            )  # 在50日高点的前20%
        ...
        macd_not_overextended = (dataframe["macd"] < 0) | (  # MACD在零轴下方
            (dataframe["macd"] > 0)
            & (dataframe["macd"] < dataframe["macd"].rolling(20).quantile(0.3))
        )  # 或在正值区域的低位
...
"""
        ...
        # === 趋势确认：只在下降趋势或横盘中做空 ===
        trend_confirmation = (dataframe["ema_8"] <= dataframe["ema_21"]) | (  # 空头排列
            dataframe["adx"] < 25
        )  # 或横盘环境
        ...
        # === 背离检测：避免在底背离时入场 ===
        no_bullish_divergence = ~self._safe_get_bool_series(
        ...
        # RSI做多信号
        rsi_long_regime_ok = "RSI_Oversold_Bounce" not in signals_advice.get(
            "avoid_signals", []
        )
        high_quality_long = (
            rsi_oversold_bounce & (rsi_long_score >= 6) & rsi_long_regime_ok
        )

        # RSI做空信号
        rsi_short_regime_ok = "RSI_Overbought_Fall" not in signals_advice.get(
            "avoid_signals", []
        )
        high_quality_short = (
            rsi_overbought_fall & (rsi_short_score >= 6) & rsi_short_regime_ok
        )

        # 市场状态奖励：在推荐的市场环境中降低质量要求
        if "RSI_Oversold_Bounce" in signals_advice.get("recommended_signals", []):
            regime_bonus_long = rsi_oversold_bounce & (
                rsi_long_score >= 5
            )  # 降低1分要求
            high_quality_long = high_quality_long | regime_bonus_long

        if "RSI_Overbought_Fall" in signals_advice.get("recommended_signals", []):
            regime_bonus_short = rsi_overbought_fall & (
                rsi_short_score >= 5
            )  # 降低1分要求
        ...
        dataframe.loc[high_quality_long, "market_regime_bonus"] = (
            "RSI_Oversold_Bounce" in signals_advice.get("recommended_signals", [])
        )
        ...
        dataframe.loc[high_quality_short, "market_regime_bonus"] = (
            "RSI_Overbought_Fall" in signals_advice.get("recommended_signals", [])
        )
        ...
        medium_death_with_confirm = (
            ...
            & (
                self._safe_get_bool_series(dataframe, "bullish_divergence", False)
                | (dataframe["rsi_14"] < 35)
            )  # 底背离或超卖
        )
        ...
        dataframe.loc[ema_death_cross_reversal, "enter_tag"] = (
            "EMA_Death_Cross_Reversal_Filtered"
        )
...
"""
        ...
        # === MACD基础信号（传统+前置） ===
        macd_death_cross = (dataframe["macd"] < dataframe["macd_signal"]) & (
            dataframe["macd"].shift(1) >= dataframe["macd_signal"].shift(1)
        )
        macd_hist_negative = (dataframe["macd_hist"] < 0) & (
            dataframe["macd_hist"].shift(1) >= 0
        )
        ...
        price_in_bottom_20pct = (
            (
                dataframe["close"] < dataframe["low"].rolling(50).quantile(0.2)
            )  # 在50日低点的前20%
        ...
        macd_not_oversold = (dataframe["macd"] > 0) | (  # MACD在零轴上方
            (dataframe["macd"] < 0)
            & (dataframe["macd"] > dataframe["macd"].rolling(20).quantile(0.7))
        )  # 或在负值区域的高位
        ...
        # 4. 强度确认：ADX显示趋势强化
        strength_confirmation = (dataframe["adx"] > 25) & (  # 有一定趋势强度
        ...
        position_confirmation = (
            dataframe["close"]
            > dataframe["close"].rolling(20).mean() * 1.02  # 价格相对偏高
        )
        ...
        # === 📊 MACD信号质量评分 ===
        macd_score = self._calculate_macd_signal_quality(
            dataframe, macd_bearish, "MACD_Bearish"
        )
        ...
        high_quality_macd = (
            macd_bearish & (macd_score >= 7) & macd_regime_ok
        )  # MACD要求更高分数+状态确认

        # 市场状态奖励：在强下跌趋势中降低MACD要求
        if "MACD_Bearish" in signals_advice.get("recommended_signals", []):
            regime_bonus_macd = (
                macd_bearish & (macd_score >= 6) & macd_regime_ok
            )  # 降低1分要求
        ...
        dataframe.loc[high_quality_macd, "market_regime_bonus"] = (
            "MACD_Bearish" in signals_advice.get("recommended_signals", [])
        )
        ...
        dataframe["leverage_multiplier"] = self._calculate_leverage_multiplier(
            dataframe
        )
...
"""
    ...
    def _calculate_trend_adjustment(
        self, current_candle: dict, is_short: bool, entry_atr_p: float
    ) -> float:
        """
        趋势强度调整 - 顺势宽松，逆势严格
        """
    ...
    def _log_stoploss_calculation(
        ...
        """
        hours_held = (
            datetime.now(timezone.utc) - trade.open_date_utc
        ).total_seconds() / 3600
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        try:
            ...
            # === 6. 最终止损计算 ===
            final_stop_distance = (
                base_stop_distance * time_decay_factor * trend_adjustment
            )
            ...
            # 做空单需要取反
            final_stoploss = (
                -final_stop_distance if not trade.is_short else final_stop_distance
            )
    ...
    def _calculate_signal_quality_score(
        self, dataframe: DataFrame, signal_mask: pd.Series, signal_type: str
    ) -> pd.Series:
        """
        🎯 智能信号质量评分系统 (1-10分)
        ...
        for idx in signal_indices:
            try:
                ...
                if (
                    signal_type in ["RSI_Oversold_Bounce"]
                    and 0.2 < price_position < 0.7
                ):
                    score += 0.5  # 不在极端位置做多
                elif (
                    signal_type in ["RSI_Overbought_Fall"]
                    and 0.3 < price_position < 0.8
                ):
                    score += 0.5  # 不在极端位置做空
    ...
    def _calculate_macd_signal_quality(
        self, dataframe: DataFrame, signal_mask: pd.Series, signal_type: str
    ) -> pd.Series:
        """
        ...
        for idx in signal_indices:
            try:
                ...
                volume_ratio = current_data.get("volume_ratio", 1.0)
                volume_trend = (
                    dataframe["volume"].iloc[max(0, idx - 3) : idx + 1].iloc[-1]
                ...
                adx = current_data.get("adx", 25)
                adx_trend = (
                    current_data.get("adx", 25) > dataframe["adx"].iloc[max(0, idx - 3)]
                )
    ...
    def _enhanced_market_regime_detection(self, dataframe: DataFrame) -> Dict[str, Any]:
        ...
        try:
            ...
            return {
                ...
                "signals_advice": self._get_regime_trading_advice(
                    regime, volatility_regime, position_regime
                ),
    ...
    def _get_regime_trading_advice(
        self, regime: str, volatility_regime: str, position_regime: str
    ) -> Dict[str, list]:
        """
        基于市场状态给出交易建议
        """
        ...
        if "TRENDING_UPTREND" in regime:
            ...
            if volatility_regime == "LOW":
                advice["recommended_signals"] = [
                    "RSI_Oversold_Bounce",
                    "RSI_Overbought_Fall",
                ]
                advice["avoid_signals"] = ["MACD_Bearish"]
            else:
                advice["avoid_signals"] = [
                    "MACD_Bearish",
                    "RSI_Overbought_Fall",
                    "RSI_Oversold_Bounce",
                ]
    ...
    def _calculate_signal_quality_leverage_bonus(
        self, entry_tag: str, current_data: dict, regime: str, signals_advice: dict
    ) -> float:
        """
        基于信号质量计算杠杆奖励倍数
        高质量信号允许更高杠杆
    ...
    def _get_signal_leverage_multiplier(
        self, entry_tag: str, signals_advice: dict
    ) -> float:
        """
        基于信号类型计算杠杆倍数
        """
    ...
    def populate_exit_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        strong_reversal_long_exit = (
            ...
            # 新增：动量衰竭或趋势末期
            (
                (dataframe["momentum_exhaustion_score"] > 0.4)
                | (dataframe["trend_phase"] >= 3)
            )
        )
        ...
        # 检测同一K线的多空信号冲突
        signal_conflict = (dataframe["enter_long"] == 1) & (
            dataframe["enter_short"] == 1
        )

        # 解决冲突：基于信号强度或市场趋势优先级
        conflict_resolution_favor_long = signal_conflict & (
        ...
        dataframe.loc[
            signal_conflict & ~conflict_resolution_favor_long, "enter_long"
        ] = 0
        ...
        dataframe.loc[
            strong_bullish_signal & (dataframe["exit_short"] == 1), "exit_tag"
        ] = "smart_cross_exit_bullish"

        dataframe.loc[
            strong_bearish_signal & (dataframe["exit_long"] == 1), "exit_tag"
...
"""
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        # 1. 基础条件：必须在亏损
        is_losing = (not trade.is_short and current_rate < trade.open_rate) or (
...
"""
    ...
    def _analyze_dca_opportunity(
        ...
        try:
            ...
            if hasattr(self, "_trade_targets") and trade.pair in self._trade_targets:
                ...
                for dca_level in dca_levels:
                    if dca_level["level"] == trade.nr_of_successful_entries + 1:
                        # 检查是否达到预设价格
                        if (
                            not trade.is_short and current_rate <= dca_level["price"]
                        ) or (trade.is_short and current_rate >= dca_level["price"]):
                            pre_calculated_dca = dca_level
                            logger.info(
                                f"📍 触及预设DCA{dca_level['level']}价格: ${dca_level['price']:.4f}"
                            )
                            break
            else:
                leverage = 10  # 默认杠杆
            ...
            # === 基础DCA触发条件（结合杠杆调整）===
            # 杠杆越高，DCA触发条件越严格
            leverage_adjusted_deviation = self.dca_price_deviation * (
                1 if leverage <= 5 else 0.8 if leverage <= 10 else 0.6
            )
            leverage_adjusted_loss = (
                -0.02 if leverage <= 5 else -0.015 if leverage <= 10 else -0.01
            )

            basic_trigger_met = (pre_calculated_dca is not None) or (  # 触及预设价格
                price_deviation
                > leverage_adjusted_deviation  # 价格偏差足够（根据杠杆调整）
                and current_profit < leverage_adjusted_loss  # 浮亏门槛（根据杠杆调整）
                and hold_hours > 0.25  # 持仓至少15分钟
            )
...
"""
            ...
            if not trade.is_short:
                ...
                oversold_dca = (
                    ...
                    and current_data.get("rsi_14", 50) < 40  # 简化：仅RSI判断，放宽到40
                )
...
"""
                ...
                overbought_dca = (
                    ...
                    and current_data.get("rsi_14", 50) > 60  # 简化：仅RSI判断，放宽到60
                )
    ...
    def _calculate_smart_dca_amount(
        ...
        try:
            ...
            if hasattr(self, "_trade_targets") and trade.pair in self._trade_targets:
                ...
                preset_multiplier = leverage_params.get("dca", {}).get(
                    "multiplier", 1.2
                )
                ...
                for dca_level in dca_levels:
                    if dca_level["level"] == entry_count:
                        preset_multiplier = dca_level.get(
                            "amount_multiplier", preset_multiplier
                        )
                        logger.info(
            ...
            if hasattr(self, "_trade_targets") and trade.pair in self._trade_targets:
                ...
                type_multiplier = dca_type_multipliers.get(
                    dca_decision["dca_type"], 1.0 * leverage_factor
                )
                logger.info(
                    f"📊 使用动态DCA金额倍数: {type_multiplier:.1f}x (杠杆调整: {leverage_factor:.1f}x)"
                )
            ...
            # 高杠杆时降低市场乘数
            market_multiplier = (
                market_base_multipliers.get(market_state, 1.0) * leverage_factor
            )

            # === 根据加仓次数递减（高杠杆时更严格）===
            # 后续加仓应该更保守，高杠杆时衰减更快
            leverage_decay_factor = (
                0.15 if leverage <= 5 else 0.20 if leverage <= 10 else 0.25
            )
...
"""
            ...
            max_ratio = leverage_max_ratios.get(
                dca_decision["risk_level"], 0.05 * leverage_factor
            )
            ...
            logger.info(
                f"💰 最终DCA金额: ${final_dca:.2f} (限制: ${max_dca_amount:.2f}, 杠杆{leverage}x)"
            )
    ...
    def _dca_risk_validation(
        ...
        try:
            ...
            if risk_check["adjusted_amount"] < min_meaningful_dca:
                ...
                risk_check["reason"] = (
                    f"提升DCA金额至最小有效金额${min_meaningful_dca:.2f}"
                )
        ...
        try:
            ...
            if not dataframe.empty:
                ...
                self._trade_targets[pair] = {
                    ...
                    "trailing_activation": leverage_params["trailing_stop"][
                        "activation_price"
                    ],
                }
    ...
    def leverage(
        ...
        **kwargs,
        ...
        try:
            ...
            # === 3. 基础杠杆计算 ===
            base_leverage = self.calculate_leverage(
                "sideways", volatility, pair, current_time
            )

            # === 4. 市场状态调整 ===
            regime_multiplier = self._get_regime_leverage_multiplier(
                regime, regime_confidence
            )

            # === 5. 信号类型调整 ===
            signal_multiplier = self._get_signal_leverage_multiplier(
                entry_tag, signals_advice
            )
