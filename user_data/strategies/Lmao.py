...
class Lmao(IStrategy):
    ...
    minimal_roi = {"0": 0.006, "15": 0.003, "30": 0}
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        informative_15m = self.dp.get_pair_dataframe(
            pair=metadata["pair"], timeframe=inf_tf_15m
        )
        ...
        dataframe = merge_informative_pair(
            dataframe, informative_15m, self.timeframe, inf_tf_15m, ffill=True
        )
    ...
    def adjust_trade_position(
        ...
        **kwargs,
        ...
        if (
            count_of_buys == 1
            and (last_candle["tpct_change_0"] > 0.001)
            ...
            and (last_candle["tpct_change_0"] > 0.001)
            and (last_candle["close"] < last_candle["open"])
            and (last_candle["ema_vwap_diff_50"] < 0.5)
        ):
...
def pmax(df, period, multiplier, length, MAtype, src):
    ...
    pmx = np.where((pm_arr > 0.00), np.where((mavalue < pm_arr), "down", "up"), "neutral")
